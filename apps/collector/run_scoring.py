...
def recalculate_scores() -> None:
    ...
    # 旧形式ID → 現行ID のマッピングを構築
    # 例: "参議院-いんどう周作[犬童周作]" → old_id "参議院-犬童周作" を生成して紐付ける
    old_to_current: dict[str, str] = {}
    for m in members:
        mid = m["id"]
        old_to_current[mid] = mid  # 現行IDはそのまま
        bracket = re.search(r"\[(.+?)\]", mid)
        if bracket:
            house = mid.split("-", 1)[0]
            old_id = f"{house}-{bracket.group(1)}"
            old_to_current.setdefault(old_id, mid)
    ...
    for s in speeches:
        raw_mid = s.get("member_id")
        if not raw_mid:
            continue
        mid = old_to_current.get(raw_mid, raw_mid)
