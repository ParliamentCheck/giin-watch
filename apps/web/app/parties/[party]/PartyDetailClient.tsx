    ...
    <div style={{ minHeight: "100vh", background: "#f4f4f4", color: "#1a1a1a",
      ...
      {tab === "ai" && (() => {
        ...
        const systemPrompt =
          ...
          "分析では単なる件数比較ではなく、この政党の「独自のシグネチャー（他党と差別化できる特徴）」を必ず3〜5個明確に抽出してください。例：「政治改革法案の共同提出が突出」「特定政党との採決一致率が極めて高く連携志向が明確」など。" +
          "提供されたベンチマーク比較・採決一致率・議員1人あたり活動量を最大限活用し、他党との相対的な立ち位置を必ず強調してください。" +
          ...
          "### 1. 政党の活動スタイルの特徴（シグネチャー）\n（リスト＋各項目に1文の考察）\n\n" +
          "### 2. 他党とのベンチマーク比較\n（テーブル＋テーブル全体の解説文）\n\n" +
          "### 3. 採決・連携傾向（現在の立場を踏まえて）\n（文章中心、データ根拠付き）\n\n" +
          "### 4. 独自提出法案の特徴と時系列変化\n（最近の動きをストーリー的に）\n\n" +
          "### 5. まとめと一言キャッチフレーズ\n（全体を振り返るまとめ文＋キャッチフレーズ）\n\n" +
          "※この分析は公開データに基づく推測であり、『はたらく議員』の公式見解ではありません。";
        const defaultQuestion =
          "提供された活動データ（ベンチマーク比較・採決一致率・議員立法・質問主意書・請願・発言キーワード・共同提出パートナーなど）から、この政党の国会活動の特徴、他党との比較、連携傾向を分析してください。特に「この政党だけが目立つシグネチャー（他党との差別化ポイント）」と2025〜2026年の再編後の直近の動きを重視し、件数の大小で優劣を判断しないでください。";
