...
export default function PartyDetailPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", color: "#e8e8e8",
      ...
      {/* ヘッダー */}
      <div style={{ background: "#141414", border: `1px solid ${color}44`,
        ...
        <div className="party-header" style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 20 }}>
          ...
          <h1 style={{ margin: 0, fontSize: 28, fontWeight: 900, color: "#f0f0f0", flex: 1 }}>{party}</h1>
          {PARTY_URLS[party] && (
            <a href={PARTY_URLS[party]} target="_blank" rel="noopener noreferrer"
              onClick={(e) => e.stopPropagation()}
              className="party-header-link"
              style={{ fontSize: 12, color: "#777777", border: "1px solid #383838",
              ...
              onMouseLeave={(e) => {
                (e.currentTarget as HTMLAnchorElement).style.color = "#777777";
                (e.currentTarget as HTMLAnchorElement).style.borderColor = "#383838";
              }}>
              公式サイト →
            </a>
          )}
        ...
        <div className="party-stats-grid">
          {[
            ...
            <div key={item.label} style={{ background: "#1e1e1e", borderRadius: 12, padding: 16, textAlign: "center" }}>
      ...
      {/* タブ */}
      <div style={{ display: "flex", gap: 4, marginBottom: 20, background: "#141414",
        border: "1px solid #1e1e1e", borderRadius: 12, padding: 4, flexWrap: "wrap" }}>
        {tabs.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{ flex: 1, minWidth: 120, padding: "10px 0", borderRadius: 9, border: "none",
              background: tab === t.id ? color : "transparent",
              color: tab === t.id ? "white" : "#777777", cursor: "pointer",
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            {[
              ...
              <button key={s.value} onClick={() => setSortBy(s.value)}
                style={{ background: sortBy === s.value ? color + "33" : "#1e1e1e",
                  border: `1px solid ${sortBy === s.value ? color : "#383838"}`,
                  color: sortBy === s.value ? color : "#777777",
            ...
            {sorted.map((m) => (
              <div key={m.id}
                ...
                style={{ display: "flex", alignItems: "center", gap: 12,
                  ...
                  background: "#1e1e1e", transition: "all 0.15s" }}
                onMouseEnter={(e) => { e.currentTarget.style.background = "#263548"; }}
                onMouseLeave={(e) => { e.currentTarget.style.background = "#1e1e1e"; }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 600, fontSize: 14, color: "#f0f0f0" }}>{m.name}</div>
                  <div style={{ fontSize: 12, color: "#777777" }}>{m.house} · {m.district}</div>
                </div>
                <div style={{ display: "flex", gap: 16, fontSize: 12, color: "#777777" }}>
      ...
      {tab === "committees" && (
        <div style={{ background: "#141414", border: "1px solid #1e1e1e", borderRadius: 12, padding: 20 }}>
          {chairList.length > 0 && (
            <>
              <h3 style={{ margin: "0 0 12px", fontSize: 13, color: "#d0d0d0",
                ...
                {chairList.map((c, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 12,
                    ...
                    <span style={{ background: "#55555522", color: "#d0d0d0",
                      border: "1px solid #55555544", padding: "2px 8px",
                      borderRadius: 4, fontSize: 11, fontWeight: 700, flexShrink: 0 }}>
                      {c.role}
                    </span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 600, fontSize: 14, color: "#f0f0f0" }}>{c.name}</div>
                      <div style={{ fontSize: 12, color: "#777777" }}>{c.committee}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {tab === "wordcloud" && (
        <div style={{ background: "#141414", border: "1px solid #1e1e1e", borderRadius: 12, padding: 24 }}>
          <h3 style={{ margin: "0 0 20px", fontSize: 13, color: "#999999",
            textTransform: "uppercase", letterSpacing: 1 }}>
            ☁️ {party} の発言キーワード
          </h3>
          {kwLoading ? (
            <div style={{ textAlign: "center", padding: "60px 0", color: "#777777" }}>
              キーワードを集計中...
            </div>
          ) : (
          ...
          <div style={{ background: "#141414", border: "1px solid #1e1e1e", borderRadius: 12, padding: 24 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
              {[
                ...
                { label: "参議院", count: sangiin, bg: "#d0d0d0" },
              ].map((h) => (
                <div key={h.label} style={{ background: "#1e1e1e", borderRadius: 10, padding: 16, textAlign: "center" }}>
                  <div style={{ fontSize: 28, fontWeight: 800, color: h.bg, marginBottom: 4 }}>
                    {h.count}
                    <span style={{ fontSize: 13, color: "#777777", marginLeft: 4 }}>名</span>
                  </div>
                  <div style={{ fontSize: 12, color: "#777777" }}>{h.label}</div>
            ...
            <div style={{ height: 10, borderRadius: 5, overflow: "hidden",
              ...
              <div style={{ flex: 1, background: "#d0d0d0" }} />
            </div>
          </div>
                ...
                return (
                  <div key={b.label}>
                    <div style={{ display: "flex", justifyContent: "space-between",
                      fontSize: 12, color: "#999999", marginBottom: 4 }}>
                      <span>{b.label}</span>
