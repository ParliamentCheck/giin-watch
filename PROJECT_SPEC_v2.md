...
| prev_party | text | 中道改革連合メンバーの旧所属政党（公明党 または 立憲民主党）。政党ネットワーク分析で正確な集計をするために追加。中道改革連合以外の議員はNULL |
...
#### タブ切り替え時の document.title 更新

タブ付きクライアントページでは、タブ切り替えのたびに `document.title` を更新してブラウザ履歴・タブ名に反映する。

```tsx
const TAB_LABELS: Record<string, string> = {
  speeches: "発言", questions: "質問主意書", votes: "採決", /* ... */
};

// tab の宣言より後に配置すること（TDZ エラー防止）
useEffect(() => {
  const tabLabel = TAB_LABELS[tab] ?? tab;
  document.title = `${memberName} — ${tabLabel} | はたらく議員`;
}, [memberName, tab]);
```

- この `useEffect` は必ず `const tab = searchParams.get("tab") ?? "..."` の**後**に配置する
- GA4 の Analytics.tsx が `pathname + searchParams` の変化を検知してページビューを自動送信するため、タブ切り替えもGA4に記録される
...
- **政党間マトリクスカード**: 全件数が多い上位8政党をドミナントとして動的選択しヒートマップ表示。列ヘッダーはスクロール時に固定（sticky）。全幅（PC）
- マトリクスの数字をクリック → そのペアの議員立法一覧にドリルダウン
- **注記**: 「与党は主に閣法を通じて法案を提出するため議員立法が少ない傾向があります」という構造的バイアス注記を表示

**中道改革連合の扱い（effectiveParty）:**
中道改革連合は旧公明党・旧立憲民主党が合流した会派（2024年〜）。`members.party === "中道改革連合"` のメンバーに対しては `prev_party`（旧所属政党）を使って集計する。これにより「中道改革連合」の内部が実態は別政党であることを正確に反映する。

```ts
effectiveParty[m.id] = m.party === "中道改革連合" && m.prev_party ? m.prev_party : m.party;
```

#### 議員詳細ページの機能

- **採決タブ（参議院のみ）**: 賛成・反対・欠席・欠席率の4ボックスを1行表示（スマホ含む）
- **採決タブのフィルター**: 賛成・反対・欠席の各カードをクリックで一覧を絞り込み。再クリックまたは「解除」で全件表示に戻る。欠席率カードはフィルター対象外（統計値のため）
- **プロンプト作成ボタン**: 議員名・政党・院・選挙区 + ページURLをAI向けプロンプト形式でクリップボードにコピー
  - プロンプト文: `{name}（{party}・{house}・{district}）について詳しく教えてください。\n{url}`
- **議員立法タブのサブタブ**: 「議員提出法案」「共同提出パートナー」の2サブタブ構成。共同提出パートナーは件数順TOP10を表示し、異なる政党の場合は「超党派」バッジを付ける。中道改革連合メンバーは `prev_party` で元の政党を考慮して判定する。
...
- Grok: `https://api.x.ai/v1/chat/completions`（OpenAI互換・同じ `callOpenAI` 関数で処理）
- Gemini: `https://generativelanguage.googleapis.com/v1beta/models/{model}:streamGenerateContent?alt=sse&key={key}`
...
**AI分析プロンプトの設計方針（議員・政党共通）:**

プロンプトは `SYSTEM_PROMPT`（固定）+ `contextText`（データ）+ `question`（ユーザー編集可）の3層構成。

`SYSTEM_PROMPT` に含めるべきルール：

1. **シグネチャー抽出**: 単なるテーマ一覧ではなく「この議員・政党だけが目立つ差別化ポイント」を3〜5個明確に抽出させる
2. **役職・動的情報の補完ルール（ハルシネーション防止）**:
   - 2026年3月現在の信頼できる公開情報（公式サイト・国会記録・大手メディア）に基づき補完可能
   - 不確かな情報・古い情報・推測は一切使用しない
   - 確認できない場合は「提供データに基づく範囲で」と記述
   - 補完情報は「2026年3月現在の公開情報に基づく」と明記
   - 議員数・役職・在籍状況は「公示前/選挙前情報」など曖昧さを明示
   - 主観的・推測的表現（「連携を強めています」等）は禁止、データ・報道の具体例で示す
   - 誤認リスクが高い項目には注記を入れる
3. **出力スタイル**: 淡々としたリストではなく「読みやすい記事風」の文章で繋げる
4. **中立性**: 断定的な評価・好意的・批判的な表現を避け、データから読み取れる傾向のみ記述
5. **採決データの留保**: 党議拘束の影響を明記。個人の傾向との乖離がある場合は指摘

`contextText` に含めるデータ（議員ページ）:
- 議員名・政党・院・選挙区・閣僚役職・当選回数・発言セッション数
- データ収集範囲（質問主意書等: 最古〜最新日付、発言: 第210回国会以降）
- 質問主意書（件数・タイトル・年）
- 議員立法（件数・タイトル・年）
- 採決集計（賛成/反対/欠席の件数・割合）+ 採決詳細（直近N件）
- 所属委員会と役割
- 請願（件数・採択/不採択/審査未了の内訳）
- 発言頻出キーワード上位15語
- 発言セッション直近15件（委員会名・日付）
- 共同提出パートナー上位（議員名・政党・件数）
...
### 超党派共同立法の個別判定（実装・削除済み）

個別法案への「超党派」ラベル表示は、データ正確性の問題により削除済みのまま。

**問題の内容:**
`members.party` は現在の所属政党のみを保持し、法案提出時点の政党を記録していない。このため「法案X は超党派か？」という判定は100%正確にできない。

**復活の条件:** 議員ごとの政党所属変遷（いつからいつまでどの政党か）をDBに持てれば実装可能。現状の設計では対応できない。

**注**: 政党ネットワーク分析（集計レベル）は別途実装済み（→ § 5.3 議員立法ページの政党ネットワーク分析）。こちらは法案単位の判定ではなく「政党ペア間の共同提出件数の合計」であり、`prev_party` による中道改革連合の補正を施した上で集計しているため、正確性の基準を満たしている。
...
- **ページ最大幅: 960px**（`maxWidth: 960, margin: "0 auto"`）。全ページ共通。
...
**各ページの description に含めるデータ（実績値）:**

| ページ | description に含める実データ |
|--------|---------------------------|
| `/members/[id]` | 議員名・政党・院・選挙区・質問主意書件数・議員立法件数・発言セッション数 |
| `/parties/[party]` | 政党名・現職議員数・質問主意書合計・議員立法合計・発言セッション合計 |
| `/committees/[name]` | 委員会名・委員長名（committee_membersから取得）・メンバー数 |
| `/bills` | 議員立法件数・閣法件数（billsテーブルからbill_typeで集計） |
