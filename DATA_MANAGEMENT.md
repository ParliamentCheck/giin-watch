...
- 議員ごとに `spoken_at` 降順で **最大30件** を保持（直近5件 + 5グループ×5件の分散取得）
...
### 委員会所属（committee_members）のゴーストレコード問題
**発生日：** 2026-03-22
**症状：** 衆院予算委員会のDB件数（70名）が公式サイトの実人数（50名）を大幅に上回っていた。
**原因：** `upsert on_conflict=(member_id, committee, role)` では、委員を退いた議員の古いレコードが削除されずに残り続ける。
**修正：** コレクター実行のたびに `DELETE WHERE house = '衆議院'`（および参議院）で全件削除してから再登録するよう変更した（`committees.py`）。
**教訓：** upsert はあくまで「追加・更新」であり「削除」は行わない。定期的に全件入れ替えが必要なテーブルは `delete → batch_upsert` の2ステップにする。

---

### 参院議員立法の廃案（未了）が審議中として表示される問題
**発生日：** 2026-03-22（バックフィル実行後に発覚）
**症状：** バックフィル実行後、法案ページの「廃案」件数が大幅に減少した。
**原因：** `scrape_sangiin_bills` が meisai（参院詳細ページ）のテキストから「未了」を検出する方式だが、第208〜213回等の旧セッションでは meisai ページに「未了」が記載されておらず、ステータスが空文字になった。空文字は `classifyStatus` で「審議中」に分類される。
**修正：** 衆院 kaiji ページ（`kaiji{session}.htm`）の「参法」テーブルにはステータス列が存在する。meisai でステータスを取得できなかった法案に対してこのページの値をフォールバックとして使用する `_fetch_sangiin_statuses_from_shugiin()` を追加した（`bills.py`）。
**修正後の対応：** バックフィルJob（`backfill.yml`）の再実行でDBを更新する。
**教訓：** スクレイパーを変更した後にバックフィルを実行すると、改悪があった場合に既存の正しいデータを上書きしてしまう。変更内容とバックフィルの影響範囲を事前に確認する。
